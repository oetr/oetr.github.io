...
#+TITLE:   Fast math in Racket (part 0)
...
#+begin_src racket :session *RR* :exports both :results value
(require fm)
(fm:arange 0 10)
#+end_src
...
#+begin_src racket :session *RR* :exports both :results output
  (pretty-print (fm:arange 0 100000))
#+end_src

#+begin_src racket :session *RR* :exports both :results value
 (make-ndarray '(2 1000 1000) 3 #:type 'uint8)
#+end_src


#+begin_src racket :session *RR* :exports both :results raw :file ../images/fast-math-001/test.png
(require racket/draw)


(define W 500)
(define H 5)
(define bm0 (make-bitmap W H #t))
(define dc (new bitmap-dc% [bitmap bm0]))
(define c0 (make-color 100 100 100))
(send dc clear)
...
(define cairo-surface (send bm0 get-handle))
(define context (cairo_create cairo-surface))
(define mem (cairo_image_surface_get_data* cairo-surface))
(define M (ndarray (list H W 4) 'uint8 mem))
M
#+end_src
